...
void loadNextFile(int _i, int _ntwsize, int _enssize, float _gamma, char *_pathgenr, float **_gnLcc, int **_geCnt) {
	// Declare Variables
	char filepathlcc[200]={}, filenamelcc[200]={}, filepathdeg[200]={}, filenamedeg[200]={};
	FILE *genrLcc, *genrDeg;
	int i = 0, j = 0, generated, numtris;
	float templcc;

	// Open files
	strcpy(filepathlcc, _pathgenr);
	//Coolen_localTri_N100r2.0Sq195.txt
	sprintf(filenamelcc, "Coolen_localTri_N%dr%.1fSq%d.txt", _ntwsize, -1*_gamma, _i);
	strcat(filepathlcc,filenamelcc);
	printf("%s\n", filepathlcc);
	if((genrLcc = fopen(filepathlcc, "r")) == NULL) {
	 	printf("Error opening %s exiting to system\n", filepathlcc);
	 	return;
	}

	strcpy(filepathdeg, _pathgenr);
	//degseq_N100r2.0Sq725.txt
	sprintf(filenamedeg, "degseq_N%dr%.1fSq%d.txt", _ntwsize, -1*_gamma, _i);
	strcat(filepathdeg,filenamedeg);
	printf("%s\n", filepathdeg);
	if((genrDeg = fopen(filepathdeg, "r")) == NULL) {
	 	printf("Error opening %s exiting to system\n", filepathdeg);
	 	return;
	}
	...
	for(i = 0; i < 10000; i++) {
		...
		for(j = 0; j < _ntwsize; j++) {
			fscanf(genrDeg, "%d", &generated);
			fscanf(genrLcc, "%d", &numtris);
