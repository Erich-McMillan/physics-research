...
void broydn(float x[], int n, int *convProb,
	void (*vecfunc)(int, float [], float []))
{
	...
	for (its=1;its<=MAXITS;its++) {
        //printf("\t\t\tBrodyn iteration = %d\n", its);
		if (restrt) {
			fdjac(n,x,fvec,r,vecfunc);
			qrdcmp(r,n,c,d,&sing);
            if (sing) {
                *convProb = 1;
                //printf("singular Jacobian in broydn\n");
                return;
		...
		rsolv(r,n,d,p);
		lnsrch(n,xold,fold,g,p,x,&f,stpmax,convProb,f_nr_min);
		test=0.0;
		for (i=1;i<=n;i++)
			if (fabs(fvec[i]) > test) test=fabs(fvec[i]);
		if (test < TOLF) {
    ...
    printf("MATXITS exceeding in broydn\n");
    *convProb = 2;
    return;
	FREERETURN
}
