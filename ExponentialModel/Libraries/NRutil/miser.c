	...
	float dith, float *ave, float *var)
{
	...
	float *fmaxl,*fmaxr,*f_nr_minl,*f_nr_minr;
	float *pt,*rmid;
	...
	if (npts < MNBS) {
		...
		*var=FMAX(TINY,(summ2-summ*summ/npts)/(npts*npts));
		...
		for (n=1;n<=npre;n++) {
			...
			for (j=1;j<=ndim;j++) {
				if (pt[j]<=rmid[j]) {
					f_nr_minl[j]=FMIN(f_nr_minl[j],fval);
					fmaxl[j]=FMAX(fmaxl[j],fval);
				}
				else {
					f_nr_minr[j]=FMIN(f_nr_minr[j],fval);
		...
		siglb=sigrb=1.0;
		for (j=1;j<=ndim;j++) {
			if (fmaxl[j] > f_nr_minl[j] && fmaxr[j] > f_nr_minr[j]) {
				sigl=FMAX(TINY,pow(fmaxl[j]-f_nr_minl[j],2.0/3.0));
				sigr=FMAX(TINY,pow(fmaxr[j]-f_nr_minr[j],2.0/3.0));
				sum=sigl+sigr;
				if (sum<=sumb) {
					sumb=sum;
					jb=j;
		...
		free_vector(f_nr_minr,1,ndim);
		free_vector(f_nr_minl,1,ndim);
