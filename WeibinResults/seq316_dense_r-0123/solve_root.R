...
for(Sq in 1:1000)
{
	fin=sprintf("degseq_N%dr%.1fSq%03d.txt",N,gamma,Sq)
	d=read.table(fin)$V1
	...
	b=s[[1]]
	#err=s[[4]]
	#diff_man=diffc(b,deg,freq)

	#rlist=c(rlist,gamma)
	#Sqlist=c(Sqlist,Sq)
	#errlist=c(errlist,err)
	#err_man_list=c(err_man_list,sum(abs(s[[2]]-diff_man)))
	
	#dout=data.frame(deg,b,freq,s[[2]],diff_man)
	dout=data.frame(deg,b,freq,s[[2]])
	dout=dout[order(deg,decreasing=T),]
	fout=sprintf("dbroot_N%dr%.1fSq%03d.txt",N,gamma,Sq)
	write.table(dout,file=fout,row.names=F,col.names=F)
}
}
#fout="errlist.txt"
#write.table(data.frame(rlist,Sqlist,errlist,err_man_list),file=fout,row.names=F)
